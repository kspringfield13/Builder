...
to execute complex, multi-step build processes.
"""
...
def cli(ctx, config, profile):
    """Builder - Autonomous Build Orchestration System"""
    ctx.ensure_object(dict)
    
    # Load configuration
    config_path = Path(config) if config else Path.home() / '.builder' / 'config.yaml'
...
def start(ctx, prompt_file, speed, resume, dry_run):
    ...
    try:
        ...
        # Start orchestrator
        orchestrator = BuildOrchestrator(config, session_manager, prompt_manager)
...
def attach(ctx, session_id):
    """Attach to a running build session"""
    config = ctx.obj['config']
    session_manager = SessionManager(config)
    ...
    if config.use_tmux:
        import subprocess
        subprocess.run(['tmux', 'attach-session', '-t', f'build-{session_id[:8]}'])
    else:
        # Stream logs
...
def init(ctx):
    ...
    click.echo("Builder initialized successfully!")
